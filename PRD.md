...
| **创建日期** | 2026-09-16 |
| **最后修订** | 2026-09-16 |
| **状态** | HA 设备已接入；HA 自动化规则逐条设计与确认中，尚未切换控制权 |
| **负责人** | Kavin |
...
* **决策：采用方案 A（逻辑控制替代，物理硬件共存）**：
  ...
  * **信号基站**：迁移期间小米中枢网关和米家自动化继续承担现有控制；全部 HA 规则核对完成并满足切换门禁后，统一停用米家自动化、启用 HA 自动化。切换后小米中枢仅承担设备连接与射频中继。
...
4. **逐条确认、统一切换**：以 HA 规则为核对对象，逐条确认目标行为、实体、条件、动作和失败边界。米家仅作为现状参照；所有 HA 规则核对完成前不启用 HA 自动化、不停用米家自动化。完成预检后统一切换并逐项验收，失败则回退。
...
### 5.3 HA 自动化逐条设计、确认与统一切换（2026-09-16）

当前 HA `configuration.yaml` 使用 `automation: !include automations.yaml`，但生产 `automations.yaml` 为 **0 行**，脚本、场景也为空。因此目前没有现成的 HA 自动化可逐条验收；本阶段应**逐条设计目标 HA 规则**，而不是把米家规则直接当成待批准的 HA 规则。HA 已接入的 `xiaomi_miot` 实体为 249 个；实体存在不等于其状态、事件、局域网路径或物理动作已通过验收。

米家极客版先前显示 28 条自动化、26 条运行中，但目前只掌握数量和少数规则细节，**尚未完整抄录 28 条规则**。第一步须逐条只读记录原规则名称、启用状态、触发、条件、动作、目标设备及异常提示，保留原始对应关系，不遗漏停用规则；第二步将完整清单转换为 HA 规则草稿，记录 HA 实体映射及无法等价实现的部分；第三步以 HA 草稿为对象逐条与用户确认，允许用户决定合并、拆分、修改或舍弃。全部确认和验证完成前不切换控制权。

| HA 规则 | 目标功能 | 设计确认 | 实体与触发验证 | 离线草稿 | HA 启用 |
| :--- | :--- | :---: | :---: | :---: | :---: |
| 小米原规则完整抄录 | 28 条，含 2 条停用规则；逐条记录触发、条件和动作 | 未完成 | 不适用 | 未编写 | 禁止 |
| HA 规则草稿与逐条确认 | 完整抄录后建立逐条映射，以全部目标功能覆盖为收口门槛 | 未开始 | 未开始 | 未编写 | 禁止 |

已登记但未验证的 HA 实体示例：厨房人体存在传感器 `sensor.linp_hb01_0530_occupancy_sensor`、光照度 `sensor.linp_hb01_0530_illumination`，以及厨房灯 `switch.bean_blns1_aaab_switch`。这些实体尚未读取实时状态或测试物理动作，不能仅凭实体名称生成可启用规则。

**切换门禁：**全部 HA 规则均获用户逐条确认、对应实体与真实设备动作完成验证、配置检查与冲突/回退方案通过后，才执行一次受控切换：停用米家自动化、启用 HA 自动化，并逐项检查实体状态、执行 Trace 和物理结果；任一关键规则失败即按预定方案回退。切换前不启动/重启 HA 服务、不启用 HA 自动化、不停用米家规则。小米中枢在切换后仍保留设备连接功能。

恢复点边界：VM 138 于 2026-09-16 完成一次在线 PBS 备份，归档与 VM 配置可读取，且该基线已标记受保护。3035 的 PVE10 两条每日 PBS 任务已纳入 138，并每小时自动追加以后新建的 PVE LXC；尚待下一轮计划备份及两处归档回读。基线备份时 QEMU Guest Agent 不响应，未做文件系统冻结或实际恢复演练；现有归档不是应用一致性或恢复成功的证明。
...
[阶段 4: HA 自动化逐条设计与统一切换] ⏳ 进行中
  ├── 只读完整抄录米家全部 28 条现有规则（包括停用规则及触发、条件、动作）
  ├── 转换为 HA 规则草稿，逐条与用户确认行为、实体、触发、边界及验收方法
  ├── 全部规则确认且通过实体、物理动作、冲突及回退预检后，受控停用米家自动化并启用 HA 自动化
  └── 切换后逐项验收并观察 48 小时日志与执行 Trace；小米中枢继续负责设备连接
